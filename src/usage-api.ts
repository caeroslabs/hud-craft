...
function getCachePath(homeDir: string): string {
  return path.join(homeDir, '.claude', 'plugins', 'hud-craft', '.usage-cache.json');
}
...
function getKeychainBackoffPath(homeDir: string): string {
  return path.join(homeDir, '.claude', 'plugins', 'hud-craft', '.keychain-backoff');
}
...
function getProxyUrl(): url.URL | null {
...
function handleResponse(res: http.IncomingMessage, resolve: (value: UsageApiResult) => void): void {
  let data = '';
      ...
      connectReq.on('connect', (_res, socket) => {
        const tlsSocket = tls.connect({
          ...
          const req = https.request({
            ...
            socket: tlsSocket,
            agent: false,
          } as https.RequestOptions, (res) => handleResponse(res, resolve));
