...
export function renderTodosLine(ctx: RenderContext): string | null {
  ...
  if (!inProgress) {
    if (completed === total && total > 0) {
      return `${green(emoji('todo_done', ctx.config?.emojiMode ?? 'minimal').trimEnd())} All todos complete ${dim(`(${completed}/${total})`)}`;
    }
    return null;
  }
  ...
  return `${yellow(emoji('todo_active', ctx.config?.emojiMode ?? 'minimal').trimEnd())} ${content} ${progress}`;
}
