...
export function renderSessionLine(ctx: RenderContext): string {
  ...
  if (DEBUG && autocompactMode === 'disabled') {
    console.error(`[hud-craft:context] autocompactBuffer=disabled, showing raw ${rawPercent}% (buffered would be ${bufferedPercent}%)`);
  }

  const bar = coloredBar(percent, ctx.config?.barWidth, ctx.config?.barStyle);
  ...
  if (display?.showUsage !== false && ctx.usageData?.planName && !providerLabel) {
    if (ctx.usageData.apiUnavailable) {
      ...
      parts.push(yellow('usage: ' + emoji('warning', ctx.config?.emojiMode ?? 'minimal') + errorHint));
    } else if (isLimitReached(ctx.usageData)) {
      const resetTime = ctx.usageData.fiveHour === 100
        ? formatResetTime(ctx.usageData.fiveHourResetAt)
        : formatResetTime(ctx.usageData.sevenDayResetAt);
      parts.push(red(emoji('warning', ctx.config?.emojiMode ?? 'minimal') + `Limit reached${resetTime ? ` (resets ${resetTime})` : ''}`));
    } else {
  ...
  if (display?.showDuration !== false && ctx.sessionDuration) {
    parts.push(dim(`${emoji('duration', ctx.config?.emojiMode ?? 'minimal')}${ctx.sessionDuration}`));
  }
