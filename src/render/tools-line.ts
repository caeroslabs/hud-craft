...
export function renderToolsLine(ctx: RenderContext): string | null {
  ...
  for (const tool of runningTools.slice(-2)) {
    ...
    parts.push(`${yellow(emoji('tools_running', ctx.config?.emojiMode ?? 'minimal').trimEnd())} ${cyan(tool.name)}${target ? dim(`: ${target}`) : ''}`);
  }
  ...
  for (const [name, count] of sortedTools) {
    parts.push(`${green(emoji('tools_done', ctx.config?.emojiMode ?? 'minimal').trimEnd())} ${name} ${dim(`×${count}`)}`);
  }
