...
import type { EmojiMode } from '../config.js';
...
export function renderAgentsLine(ctx: RenderContext): string | null {
  ...
  const emojiMode = ctx.config?.emojiMode ?? 'minimal';
  for (const agent of toShow) {
    lines.push(formatAgent(agent, emojiMode));
  }

  return lines.join('\n');
}

function formatAgent(agent: AgentEntry, emojiMode: EmojiMode): string {
  const statusIcon = agent.status === 'running' ? yellow(emoji('agents_running', emojiMode).trimEnd()) : green(emoji('agents_done', emojiMode).trimEnd());
