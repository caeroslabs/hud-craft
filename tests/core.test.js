...
test('parseTranscript tolerates malformed lines', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'hud-craft-'));
...
test('countConfigs honors project and global config locations', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'hud-craft-home-'));
  const projectDir = await mkdtemp(path.join(tmpdir(), 'hud-craft-project-'));
...
// https://github.com/jarrodwatts/hud-craft/issues/3
